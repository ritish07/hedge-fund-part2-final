...
def get_ai_decision(market_data):
    if not config.DEEPSEEK_API_KEY or config.DEEPSEEK_API_KEY == "sk-your-api-key":
        # Mock Response for Mac Testing if no API key is provided
        return {
            "signal": "BUY",
            "confidence_score": 85,
            "logic": "The Daily trend is bullish, and 1H structure just broke above a major resistance level. Momentum is strong."
        }
    ...
    system_prompt = """
    You are an elite quant trader. Analyze the macro and micro market structure (higher highs, support/resistance).
